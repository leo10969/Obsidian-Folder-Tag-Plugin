import { App, Editor, MarkdownView, Modal, Notice, Plugin, PluginSettingTab, Setting, TFolder, TFile, TagSuggest } from 'obsidian';
...
class TagSelectionModal extends Modal {
	...
	onOpen() {
		...
		// 既存のタグを取得
		const existingTags = this.app.metadataCache.getTags();
		const tagList = Object.keys(existingTags).map(tag => tag.slice(1)); // #を除去
	...
	async onload() {
		...
		const appendTagsToFile = async (file: TFile, tags: string[]) => {
			try {
				...
				// 新しいタグを追加
				const newTags = tags.filter(tag => !content.includes(`#${tag}`));
				if (newTags.length === 0) {
			...
			this.app.vault.on('create', (file) => {
				...
				if (file instanceof TFile) {
					...
					if (folder) {
						...
						// タグ選択モーダルを表示
						new TagSelectionModal(this.app, (selectedTags) => {
							// 選択されたタグとフォルダ名のタグを追加
							const allTags = [...new Set([...selectedTags, folder.name])];
							appendTagsToFile(file, allTags);
