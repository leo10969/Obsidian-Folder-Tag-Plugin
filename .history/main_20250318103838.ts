	...
	async onload() {
		...
		const appendTagsToFile = async (file: TFile, tags: string[]) => {
			try {
				...
				// タグを追加（ファイルの先頭に追加）
				const newContent = `${newTags.map(tag => `#${tag.replace(/^#/, '')}`).join(' ')}\n\n${content}`;
				
				// ファイルを更新
				await this.app.vault.modify(file, newContent);
				
				console.log(`Added tags ${newTags.map(tag => `#${tag.replace(/^#/, '')}`).join(', ')} to file: ${file.path}`);
		...
		const moveFileToTagFolder = async (file: TFile, tag: string) => {
			try {
				...
				const targetFolder = this.app.vault.getAllLoadedFiles().find(
					(f): f is TFolder => f instanceof TFolder && f.name === tag
				);
				...
				if (targetFolder) {
					...
					new Notice(`ファイルを #${tag} フォルダに移動しました`);
				} else {
					console.log(`No folder found matching tag: ${tag}`);
				}
			} catch (error) {
